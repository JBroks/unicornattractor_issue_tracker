...
class TestThread(TestCase):
        ...
        def test_thread_creation(self):
            ...
            displayed_object_name = f'Forum thread #{new_thread.id} ' \
                f'[{new_thread.subject}] added by {new_user.username}'
        ...
        def test_thread_votes_count_method(self):
            ...
            '''
            # Create two users
            new_user = User.objects.create_user({'username': 'joanna',
                                                 'email': 'joanna@example.com',
                                                 'password': 'secret'})
            second_user = User.objects.create_user({
                                'username': 'suzanne',
            ...
            # Create thread votes
            thread_vote_1 = ThreadVote.objects.create(
                user=new_user,
                thread=new_thread,
                vote_type='like')
            thread_vote_2 = ThreadVote.objects.create(
                user=new_user,
                thread=second_thread,
                vote_type='dislike')
            thread_vote_3 = ThreadVote.objects.create(
                user=second_user,
                thread=new_thread,
                vote_type='like')
            ...
            expected_likes_count_second_thread = second_thread\
                .thread_likes_count()
            self.assertEquals(expected_likes_count_second_thread, 0)
            
            # Test dislikes count
            expected_dislikes_count_new_thread = new_thread\
                .thread_dislikes_count()
            self.assertEquals(expected_dislikes_count_new_thread, 0)
            expected_dislikes_count_second_thread = second_thread\
                .thread_dislikes_count()
...
class TestPost(TestCase):
        ...
        def test_post_creation(self):
            ...
            displayed_object_name = f'Post #{new_post.id} added by '\
                f'{new_user.username} for thread #{new_thread.id}'
        ...
        def test_post_votes_method(self):
            ...
            # Create post votes
            post_vote_1 = PostVote.objects.create(
                user=new_user,
                post=new_post,
                vote_type='like')
            post_vote_2 = PostVote.objects.create(
                user=new_user,
                post=second_post,
                vote_type='dislike')
            post_vote_3 = PostVote.objects.create(
                user=second_user,
                post=new_post,
                vote_type='like')
            ...
            expected_dislikes_count_second_post = second_post\
                .post_dislikes_count()
...
class TestThreadVotes(TestCase):
        ...
        def test_thread_vote_creation(self):
            ...
            thread_vote = ThreadVote.objects.create(
                user=new_user,
                thread=new_thread,
                vote_type='like')
            # Retrive the vote
            vote = ThreadVote.objects.get(id=1)
            displayed_object_name = f'Thread #{vote.thread.id} ' \
                f'{vote.vote_type}d by {vote.user.username}'
...
class TestPostVotes(TestCase):
        ...
        def test_post_vote_creation(self):
            ...
            displayed_object_name = f'Post #{vote.post.id} ' \
                f'{vote.vote_type}d by {vote.user.username}'
            self.assertEquals(displayed_object_name, str(vote))
