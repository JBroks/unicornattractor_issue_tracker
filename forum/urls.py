...
from forum.views import forum, add_or_edit_thread, delete_thread, view_thread, add_or_edit_post, delete_post, vote_thread, vote_post
...
urlpatterns = [
    ...
    url(r'^view/thread/(?P<thread_pk>\d+)/vote/(?P<vote_type>\w+)/$', vote_thread, name="vote_thread"),
    url(r'^view/thread/(?P<thread_pk>\d+)/posts/vote/(?P<post_pk>\d+)/(?P<vote_type>\w+)/$', vote_post, name="vote_post"),
