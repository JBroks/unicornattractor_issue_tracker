    ...
    if type_filter_query and type_filter_query != 'Select...':
        if status_filter_query and status_filter_query != 'Select...':
            qs = qs.filter(ticket_type__iexact=type_filter_query,
                           ticket_status__iexact=status_filter_query).order_by(
                               'ticket_type')
        else:
            qs = qs.filter(ticket_type__iexact=type_filter_query)
...
def view_ticket(request, pk):
    '''
    Enables user to view page containing all details regarding
    a selected ticket.
    ...
    user_commented_count = len(Comment.objects.filter(
        ticket=ticket).order_by().values_list('user', flat=True).distinct())
    ...
    try:
        last_comment = Comment.objects.filter(
            ticket=ticket).latest('date_created')
    except Comment.DoesNotExist:
        last_comment = None
...
def upvote(request, pk):
    ...
    downvote and upvote ticket again for the same ticket, so function will
    check it by using has_voted and has_donated.
...
def delete_comment(request, ticket_pk, comment_pk):
    '''
    View allows the user to delete their comment.
    Error message will be displayed if a user tries to delete a comment that
